...
type Props = {
  title: string;
  cards: FlashcardModeCard[];
  userId: string;
  onExit: () => void;
};

export function FlashcardMode({ title, cards, userId, onExit }: Props) {
  ...
  async function rate(remembered: boolean) {
    if (!card) return;
    // Simple SM-2-ish spaced repetition update (unchanged from the original study flow).
    ...
    if (index + 1 < cards.length) {
      ...
      const res = await recordActivity(userId, "study", title, xp + known * 2);
    ...
    <div className="mx-auto max-w-2xl">
      ...
      >
        ...
        {card.image_url && !flipped && (
          <img src={card.image_url} alt="" className="mt-3 max-h-40 rounded-xl object-contain" />
        )}
